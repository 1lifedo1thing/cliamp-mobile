...
import stream.cliamp.mobile.data.db.SftpDao
import stream.cliamp.mobile.data.db.SftpIndexEntity
import stream.cliamp.mobile.data.db.SftpTrackEntity

private const val TAG = "cliamp/sftp"

/**
 * The scanned contents of every SSH account, and the scans that fill them.
 *
 * A singleton for the same reason [stream.cliamp.mobile.data.StationArtSource]
...
object SftpLibrary {
    ...
    private val dao: SftpDao by lazy { CliampDatabase.get(appContext).sftp() }
