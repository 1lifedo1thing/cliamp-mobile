# cliamp mobile — design system

Terminal soul, native body. One monospace face, hairline rules instead of cards, phosphor
green as the only accent, and controls with real mechanical travel. Everything below is what
`Cliamp Mobile.dc.html` actually uses — treat it as the source of truth when adding screens.
